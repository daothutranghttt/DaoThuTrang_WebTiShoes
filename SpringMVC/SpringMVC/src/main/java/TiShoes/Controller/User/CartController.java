	...
	public void add_to_cart(@PathVariable String id, HttpServletRequest request, HttpServletResponse response) {
		...
		if (a.length > 2) {
			if(process != null) {
				...
				String id_prod = a[1];
				String qty = a[2];
				int id_color_size = color_sizeService.get_Color_size_id(Integer.parseInt(process.split("_")[2]), Integer.parseInt(process.split("_")[1]), Integer.parseInt(process.split("_")[0]));
				if (id_user != "" && id_prod != "") {
