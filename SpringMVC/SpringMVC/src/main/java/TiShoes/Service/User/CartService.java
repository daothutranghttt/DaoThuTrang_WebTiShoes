...
public class CartService implements CartRepository {
	...
	public String delete_prod_in_cart_not_login(String id_prod, String list_prod) { // string list_prod format =
																					// {csid}/{csid}/{csid}...
		String newListProd = "";
		String arr[] = list_prod.split("/");
		for (int i = 0; i < arr.length; i++) {
	...
	public String change_color_in_cart_not_login(String color, String list_prod) {
		...
		if (arr.length > 2) {
			int csid = color_sizeService.get_Color_size_id_by_prodid_color_id(Integer.parseInt(arr[1]),
	...
	public String change_size_in_cart_not_login(String size, String list_prod) {
		...
		if (arr.length > 3) {
			int csid = color_sizeService.get_Color_size_id(Integer.parseInt(arr[0]), Integer.parseInt(arr[1]),
	...
	public boolean insertIntoCartDB(int quantity, int color_size_id, int user_id) {
		try {
			...
			color_sizeService = new Color_sizeService();
			Color_size p = color_sizeService.getByIdCS(color_size_id);
