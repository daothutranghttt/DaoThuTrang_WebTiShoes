...
public class OrderService implements OrderRepository {
	...
	public boolean insertIntoOrder(String fullname, String email, String phone_number, String address, int voucher_id,
		...
		try {
			...
			String sql = "INSERT INTO `order_`(`fullname`, `email`, `phone_number`, `address`, `order_date`, `updated_at`, `voucher_id`, `note`, `status_id`, `method`, `bill`, `discount_at`, `request`) "
					+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public int get_last_order_id_by(String phone_number, String email) {
		List<Order_> o = getAllOrder();
		Collections.sort(o, new Comparator<Order_>() {
			@Override
			public int compare(Order_ o1, Order_ o2) {
	...
	public boolean confirm_request(int order_id) {
		try {
			connectService = new ConnectService();
			Connection conn = connectService.getConnect();
			String query = "update order_ set request = 1 where id = ?";
