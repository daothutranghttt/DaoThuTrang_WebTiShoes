...
<body>
			...
			<div class="nav-history">
				...
				<div>
					<img
						src="<c:url value="/assets/images/icons/icons8-search-client-60.png"/>"
						alt=""> <a
						href="/SpringMVC/admin/search-order">Search
						order</a>
				</div>
			</div>
						...
						<c:forEach var="liOrder" items="${ listOrder }" varStatus="index">
							<tr>
								...
								<c:if
										...
										<button class="btn-confirm-order confirm-order" id="${ liOrder.id }">
											<img
									...
									<c:if
										test="${ liOrder.status.id == 2 }">
										<button class="btn-confirm-order packing-order" id="${ liOrder.id }">
											<img
												src="<c:url value="/assets/images/icons/icons8-open-delivered-box-100.png"/>"
												alt="">
										</button>
									</c:if>
									<c:if test="${ liOrder.status.id == 3 && liOrder.request != 2 }">
									...
									<c:if
										test="${ liOrder.status.id == 3 && liOrder.request == 2}">
										<button class="btn-confirm-order print-order" id="${ liOrder.id }">
											<img
												src="<c:url value="/assets/images/icons/icons8-send-to-printer-40.png"/>"
												alt="">
										</button>
	...
	<script>
		...
		$('.print-order').click(function() {
			...
			xhr.open("GET", "/SpringMVC/admin/order-management?bill="+id_order);
			xhr.onload = function() {
				window.location.assign("/SpringMVC/admin/order-management/bill/"+id_order);
			};
			xhr.send();
		});
	</script>
						...
						function() {
							...
							xhr.onload = function() {
								if (url.includes("?sortby")) {
									...
									window.location
											.assign("/SpringMVC/admin/search-order?sortby=id-asc");
								}
							};
							xhr.send();
						});
						...
						function() {
							...
							xhr.onload = function() {
								if (url.includes("?sortby")) {
									...
									window.location
											.assign("/SpringMVC/admin/search-order?sortby=email-desc");
								}
							};
							xhr.send();
						});
						...
						function() {
							...
							xhr.onload = function() {
								if (url.includes("?sortby")) {
									...
									window.location
											.assign("/SpringMVC/admin/search-order?sortby=fullname-desc");
								}
							};
							xhr.send();
						});
						...
						function() {
							...
							xhr.onload = function() {
								if (url.includes("?sortby")) {
									...
									window.location
											.assign("/SpringMVC/admin/search-order?sortby=address-desc");
								}
							};
							xhr.send();
						});
						...
						function() {
							...
							xhr.onload = function() {
								if (url.includes("?sortby")) {
									...
									window.location
											.assign("/SpringMVC/admin/search-order?sortby=order-date-desc");
								}
							};
							xhr.send();
						});
						...
						function() {
							...
							xhr.onload = function() {
								if (url.includes("?sortby")) {
									...
									window.location
											.assign("/SpringMVC/admin/search-order?sortby=status-desc");
								}
							};
							xhr.send();
						});
							...
							if (txt == "") {
								...
								xhr.onload = function() {
									if (url.includes("?search")) {
										...
										window.location
												.assign("/SpringMVC/admin/search-order?search="
														+ txt);
									}
								};
								xhr.send();
		...
		$('.editneworder')
				.click(
						function() {
							...
							xhr.onload = function() {
								window.location
										.assign("/SpringMVC/admin/order-management/edit-order?id_order="
												+ id_order);
							};
							xhr.send();
						});
		$('.btn-details-order')
				.click(
						function() {
							...
							xhr.onload = function() {
								window.location
										.assign("/SpringMVC/admin/order-management/order-details?id_order="
												+ id_order);
							};
							xhr.send();
						});
