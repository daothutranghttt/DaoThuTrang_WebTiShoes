			...
			<div class="dropdown-menu" aria-labelledby="dropdownMenuButton"
				...
				<div>
					...
					<a class="dropdown-item"
						href="/SpringMVC/personal-details/${ id_user }">Personal
						details</a> <a class="dropdown-item" onclick="logout()">Logout</a>
				</div>
			</div>
		</c:if>
		<c:if test="${ userID == null }">
	...
	<nav>
		<div class="content">
			<ul class="links">
					...
					<ul>
						<c:if test="${ userID == null }">
							<c:forEach var="li_style" items="${ style }" varStatus="index">
								<c:if test="${ index.getIndex() < 9 }">
									<li><a class="menu"
										href="/SpringMVC/products?stylename=${ li_style.id }">${ li_style.style_name }</a></li>
								</c:if>
							</c:forEach>
						...
						<c:if test="${ userID != null }">
							<c:forEach var="li_style" items="${ style }" varStatus="index">
								<c:if test="${ index.getIndex() < 9 }">
									<li><a class="menu"
										href="/SpringMVC/products/${ id_user }?stylename=${ li_style.id }">${ li_style.style_name }</a></li>
								</c:if>
							</c:forEach>
							<li><a href="" class="desktop-link menu">More</a>
								<ul class="more-shoes">
				...
				<c:if test="${ userID != null }">
					<li><a class="menu" href="/SpringMVC/voucher/${id_user }"><b>Voucher</b></a></li>
				</c:if>
				<c:if test="${ userID != null }">
