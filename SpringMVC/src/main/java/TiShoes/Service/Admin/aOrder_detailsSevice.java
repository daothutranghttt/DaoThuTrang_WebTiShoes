...
public class aOrder_detailsSevice implements aOrder_detailsRepository{
	...
	public int get_qty_by_order_id(int id) {
		int qty =0;
		List<Order_details> li = getOrder_detailsByIdOrder(id);
	...
	public double get_revenue_by_order_id(int id) {
		aOrderService os = new aOrderService();
		double qty =0;
		List<Order_details> li = getOrder_detailsByIdOrder(id);
		for (Order_details o : li) {
			qty += o.getPrice_at() * o.getQuantity();
		}
		if (qty < 50) {
			qty += 11.0;
		}
		return qty - os.get_discount_by_order_id(id);
	}
