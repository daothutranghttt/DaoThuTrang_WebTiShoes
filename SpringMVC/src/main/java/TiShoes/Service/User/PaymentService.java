...
public class PaymentService {
	...
	public String authorizePayment(String product, float voucher, float subtotal, int user_id, String cartid,
			String vccode) throws PayPalRESTException {
	...
	public String authorizePaymentBuyNow(String product, float voucher, float subtotal, int user_id, int cs,
			String vccode) throws PayPalRESTException {
		userService = new UserService();
		Payer payer = getPayerInformation(userService.get_user_by_id(user_id));
		RedirectUrls redirectUrls = getRedirectURLs(user_id);
	...
	private List<Transaction> getTransactionInformation(String product, float voucher, float subtotal, String cartid,
			String vccode) {
		...
		for (Integer i : hm.keySet()) {
			...
			if (p.getDiscount() > 0) {
				...
				System.out.println(
						p.getPrice() - p.getPrice() * p.getDiscount() / 100 + "=============================22");
	...
	private List<Transaction> getTransactionInformationBN(String product, float voucher, float subtotal, int cs,
			String vccode) {
		productService = new ProductService();
		double total = 0.0;
		HashMap<Integer, String> hm1 = get_product_Id_and_quantity(product);
		for (Integer i : hm1.keySet()) {
			Product p = productService.getProduct(i);
			if (p.getDiscount() > 0) {
				total += (double) Math.round(
						(p.getPrice() - p.getPrice() * p.getDiscount() / 100) * 100)
						/ 100;
				total = total * Integer.parseInt(hm1.get(i));
			} else {
				total = (double) Math.round(p.getPrice()*100)/100 * Integer.parseInt(hm1.get(i));
			}
		}
		System.out.println("aaaaaaaaaaaaaaa" + total);
		if (subtotal < 50) {
			total = total + 11;
		}
		if (voucher > 0) {
		...
		for (Integer i : hm.keySet()) {
			...
			if (p.getDiscount() > 0) {
				item.setPrice(String.format("%.2f",
						(double) Math.round((p.getPrice() - p.getPrice() * p.getDiscount() / 100) * 100) / 100));
			} else {
				item.setPrice(String.format("%.2f", p.getPrice()));
			}
			item.setQuantity(hm.get(i));
	...
	public static void main(String[] args) {
		System.out.println((float) Math.round(80.465 * 100) / 100);
	}
}