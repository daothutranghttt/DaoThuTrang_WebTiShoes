		...
		<section class="admin-order-search">
			...
			<div class="nav-history">
				...
				<div>
					<img
						src="<c:url value="/assets/images/icons/icons8-search-client-60.png"/>"
						alt=""> <a href="/SpringMVC/admin/search-order">Search
						order</a>
				</div>
			</div>
						...
						<c:forEach var="liOrder" items="${ listOrder }" varStatus="index">
							<tr>
								...
								<c:if test="${ liOrder.status.id == 1 }">
									<td class="bg-purple">${ liOrder.status.status_name }</td>
								</c:if>
								<c:if test="${ liOrder.status.id == 2 }">
									<td class="bg-darkgray">${ liOrder.status.status_name }</td>
								</c:if>
								<c:if test="${ liOrder.status.id == 3 }">
									<td class="bg-brown">${ liOrder.status.status_name }</td>
								</c:if>
								<c:if test="${ liOrder.status.id == 4 }">
									<td class="bg-darkblue">${ liOrder.status.status_name }</td>
								</c:if>
								<c:if test="${ liOrder.status.id == 5 }">
									<td class="bg-green">${ liOrder.status.status_name }</td>
								</c:if>
								<c:if test="${ liOrder.status.id == 6 }">
									<td class="bg-red">${ liOrder.status.status_name }</td>
								</c:if>
		...
		$('.print-order')
				.click(
						function() {
							...
							xhr.open("GET",
									"/SpringMVC/admin/order-management?bill="
											+ id_order);
							xhr.onload = function() {
								window.location
										.assign("/SpringMVC/admin/order-management/bill/"
												+ id_order);
							};
							xhr.send();
						});
	...
	<script>
		...
		$('.packing-order').click(
				function() {
					...
					xhr.onload = function() {
						window.location
								.assign("/SpringMVC/admin/order-management");
					};
					xhr.send();
				});
	</script>
