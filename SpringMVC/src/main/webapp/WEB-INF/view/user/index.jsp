...
<body style="color: white;">
	...
	<header class="container-xl header-content">
			...
			<div class="slider">
				<div class="slides">
					...
					<div class="slide first">
						<img src="<c:url value="/assets/images/slide/bag2.jpg"/>" alt="">
					</div>
					...
					<div class="products-el">
						<c:if test="${ userID == null }">
							<c:if test="${ it.discount > 0 }">
								<a
									href="/SpringMVC/product-details?product-id=${ it.id }">
						...
						<c:if test="${ userID != null }">
							<c:if test="${ it.discount > 0 }">
								<a
									href="/SpringMVC/product-details/${ id_user }_${ it.id }">
									<div class="img-product-products zoom zoo" id="ex1"
										name="${it.discount }%" style="">
										<img
											src="<c:url value="/assets/images/products/${ it.thumbnail }"/>"
						...
						<div class="content-product-products">
							...
							<div class="name-product-products">
								<c:if test="${ userID != null }">
									<a
										href="/SpringMVC/product-details/${ id_user }_${ it.id }">${ it.title }</a>
								</c:if>
								<c:if test="${ userID == null }">
									<a
										href="/SpringMVC/product-details?product-id=${ it.id }">${ it.title }</a>
								</c:if>
							</div>
								...
								<div class="effect">
									<c:if test="${ userID != null }">
										<c:if test="${ item.discount > 0 }">
											<a
												href="/SpringMVC/product-details/${ id_user }_${ item.id }">
									...
									<c:if test="${ userID == null }">
										<c:if test="${ item.discount > 0 }">
											<a
												href="/SpringMVC/product-details?product-id=${ item.id }">
												<div class="zoom-most-loved zoo"
													id="ee${ index.getIndex() }" name="${item.discount }%">
													<img class="product-effect"
														src="<c:url value="/assets/images/products/${ item.thumbnail }"/>" />
									...
									<div class="content-effect">
										<div class="details">
											<a class="product-link-effect"
												href="/SpringMVC/product-details?product-id=${ item.id }"><p
		...
		<div class="section-1">
			<div class="h">BEST SELLING</div>
			<div class="position-product container-xl">
				<c:forEach var="it" items="${ listNewArrivals1 }" varStatus="">
					<div class="products-el">
						<c:if test="${ userID == null }">
							<c:if test="${ it.discount > 0 }">
								<a
	...
	function logout() {
		$(document).ready(function(){
			var xhr = new XMLHttpRequest();
			xhr.open("GET",
					"/SpringMVC/home?logout=true");
			xhr.onload = function() {
				window.location.assign("/SpringMVC/");
			};
			xhr.send();
		});
	}
