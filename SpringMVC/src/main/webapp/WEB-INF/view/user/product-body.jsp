...
<c:if test="${ userID != null}">
				...
				<div class="cost-products">
					...
					<div class="cost-table">
						<table>
							...
							<tr>
								<td class="radio"><c:if test="${ checkedPrice == '25_50' }">
							...
							<tr>
								<td class="radio"><c:if
										...
										<input type="radio" name="filterbycost" id="4" value="100_250"
											onclick="price(this)" checked>
		...
		<div class="rate-list">
			<div class="line1-rate-product">
				...
				<c:if test="${ checkedRate == 'onestar' }">
					<span class="sizeColor active" onclick="getRate(this)" id="onestar">1<img
				...
				<c:if test="${ index.getIndex() < 9 }">
					<div class="line">
						...
						<c:if test="${ empty checkedColor[liColor.id] }">
							<span onclick="getColor(this)" id="${ liColor.id }" class="color"
								style="background: ${ liColor.rgb };"></span>
		...
		<div class="gender-list">
			<div class="line1-gender-product">
				...
				<c:if test="${ checkedGender == 'women' }">
					...
					<span class="sizeColor active" onclick="getGender(this)" id="women">Women</span>
				</c:if>
	...
	<!-- LIST PRODUCTS -->
	<c:if test="${ listProductsEmpty != null && listhmp == null}">
...
<script> // add to cart
		...
		if(id_user != "") {
			$(".add-to-cart").click(function() {
				...
				xhr.open("GET",
						"/SpringMVC/add-to-cart/"+id_user+"_"+id_prod);
				xhr.onload = function() {
					window.location.assign("/SpringMVC/cart/"+id_user);
				};
				xhr.send();
			});
		} else {
			$(".add-to-cart").click(function() {
				...
				var xhr = new XMLHttpRequest();
				xhr.open("GET",
						"/SpringMVC/cart?id="+id_prod);
				xhr.onload = function() {
					window.location.assign("/SpringMVC/cart");
				};
				xhr.send();
			});
		}
...
<script> // buy now
		...
		if(id_user != "") {
			$(".buy-now").click(function(e) {
				...
				var xhr = new XMLHttpRequest();
				xhr.open("GET",
						"/SpringMVC/cart/checkout/save/"+id_user+"_"+id_prod);
				xhr.onload = function() {
					window.location.assign("/SpringMVC/cart/checkout/"+id_user+"_"+id_prod);
				};
				xhr.send();
			});
			...
			$(".buy-now").click(function(e) {
				...
				var xhr = new XMLHttpRequest();
				xhr.open("GET",
						"/SpringMVC/cart/checkout/save/"+id_prod);
				xhr.onload = function() {
					window.location.assign("/SpringMVC/cart/checkout/"+id_prod);
				};
				xhr.send();
			});
...
<script> // click name style in product
		...
		$("div.brand-product-products").click(
			function() {
				...
				xhr.onload = function() {
					if(id_user == 0) {
						window.location.assign("/SpringMVC/products?stylename="+ id_style);
					} else {
						window.location.assign("/SpringMVC/products/"+id_user+"?stylename="+ id_style);
					}
...
<script> // click name product
		...
		$("div.name-product-products").click(
			function() {
				...
				if(id_user != "") {
					xhr.open("GET",
							"/SpringMVC/product-details/"+id_user+"_"+ id_prod);
					xhr.onload = function() {
						window.location
								.assign("/SpringMVC/product-details/"+id_user+"_"+ id_prod);
					};
				} else {
					xhr.open("GET",
							"/SpringMVC/product-details?product-id="
									+ id_prod);
					xhr.onload = function() {
						window.location
								.assign("/SpringMVC/product-details?product-id="
										+ id_prod);
					};
				}
				xhr.send();
...
<script type="text/javascript"> // reset menu
		...
		function resetAll() {
			...
			xhr.onload = function() {
				if(id_user != 0) {
					window.location.assign("/SpringMVC/products/"+id_user);
				} else {
					window.location.assign("/SpringMVC/products");
				}
