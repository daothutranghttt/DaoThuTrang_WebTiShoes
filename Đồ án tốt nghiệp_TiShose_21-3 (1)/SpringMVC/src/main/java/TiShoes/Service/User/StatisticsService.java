...
public class StatisticsService {
	...
	public boolean update_order_revenue_product_num_in_statistics_DB(int prod_num, double revenue_num) {
		try {
			...
			String query = "UPDATE `statistics` SET `order_num`= `order_num` + 1, `product_num`= `product_num` + ?, `revenue_num`= `revenue_num` + ?,`updated_order`= ?, `updated_product`= ?, `updated_revenue`= ? WHERE `date` = ?";
